...
class Auth extends CI_Controller {
	...
	function login()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				redirect($this->session->flashdata('redirect') ? $this->session->flashdata('redirect') : 'auth');
			}
			else
			{
	...
	function change_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			$this->data['breadcrumb'] = array(
				anchor($this->controller.'/my', 'Meus dados'),
				$this->data['title'],
			);
			$this->load->view('admin/index', $this->data);
		}
	...
	function my()
	{
		...
		$d = new User();
		$this->data['data'] = $d->where('id', $this->session->userdata('user_id'))->get();
		...
		$this->data['url'] = $this->controller.'/my';
	...
	{
		$this->data['title'] = "Cadastrar usuário";
	...
	{
		$this->data['title'] = "Alterar usuário";
		...
		if (isset($_POST) && !empty($_POST))
		{
			...
			if ($this->input->post('password'))
			{
				...
				$this->form_validation->set_rules('password_confirm', 'Confirmação da senha', 'required');
			...
			if ($this->form_validation->run() === TRUE)
			{
				...
				//check to see if we are creating the user
				//redirect them back to the admin page
				$this->session->set_flashdata('success', "Usuário salvo");
				redirect("auth", 'refresh');
			}
		}
	...
	function create_group()
	{
		...
		$this->data['breadcrumb'] = array(
			anchor($this->controller.'/groups', 'Grupos'),
			$this->data['title'],
		);

		//validate form input
		$this->form_validation->set_rules('group_name', 'nome do grupo', 'required|alpha_dash|xss_clean');
		$this->form_validation->set_rules('description', 'descrição', 'xss_clean');
	...
	function edit_group($id)
	{
		...
		//validate form input
		$this->form_validation->set_rules('group_name', 'nome do grupo', 'required|alpha_dash|xss_clean');
		$this->form_validation->set_rules('group_description', 'descrição', 'xss_clean');
		...
		if (isset($_POST) && !empty($_POST))
		{
			if ($this->form_validation->run() === TRUE)
			{
				...
				if($group_update)
				{
					$this->session->set_flashdata('success', "Grupo salvo");
				}
				else
				{
					$this->session->set_flashdata('fail', $this->ion_auth->errors());
	...
	/**
	 * Página com mensagem de acesso negado
	 */
	public function denied()
	{
		$this->data['title'] = 'Acesso negado';
		$this->data['page'] = 'auth/denied';
	...
	public function groups()
	{
		...
		$this->data['linkactive'] = 'grupos';
		$this->data['title'] = 'Grupos';
		$this->data['page'] = 'admin/lista';

		$this->data['breadcrumb'] = array(
			anchor($this->controller.'/grupos', 'Grupos'),
		);

		$g = new Group();
		$this->data['dados'] = $g->get();
		...
		$this->data['link_editar'] = 'edit_group';
		$this->data['link_excluir'] = 'delete_group';
